...
exports.md5 = function(input) {
	...
	*/
	function buffer2binl(buffer) {
		var bin = [];
		var mask = (1 << 8) - 1;
		for(var i = 0; i < buffer.length * 8; i += 8)
		bin[i>>5] |= (buffer[i / 8] & mask) << (i%32);
		return bin;
	}
	...
	*/
	...
	return binl2hex(core_md5(buffer2binl(input), input.length * 8));
};
...
exports.sha1 = function(input) {
	...
	*/
	...
	/*
	* Convert a buffer to an array of big-endian words
	*/
	function buffer2binb(buffer) {
		var bin = Array();
		var mask = (1 << 8) - 1;
		for (var i = 0; i < buffer.length * 8; i += 8) {
			bin[i>>5] |= (buffer[i / 8] & mask) << (32 - 8 - i%32);
		}
		return bin;
	}
	...
	*/
	...
	return binb2hex(core_sha1(buffer2binb(input), input.length*8));
};
