...
XMLSerializer.prototype._serializeNode = function(node, depth) {
	var map = {};
	map[Node.DOCUMENT_NODE] = this._serializeDocument;
	map[Node.ELEMENT_NODE] = this._serializeElement;
	map[Node.ATTRIBUTE_NODE] = this._serializeAttribute;
	map[Node.TEXT_NODE] = this._serializeText;
	map[Node.CDATA_SECTION_NODE] = this._serializeCDATASection;
	map[Node.PROCESSING_INSTRUCTION_NODE] = this._serializeProcessingInstruction;
	map[Node.COMMENT_NODE] = this._serializeComment;
	
	if (node.nodeType in map) {
		return map[node.nodeType].call(this, node, depth);
	} else {
...
XMLSerializer.prototype._serializeElement = function(element, depth) {
	...
	if (element.namespaceURI) {
		var xmlns = element.getAttributeNode("xmlns");
		if (!xmlns) { arr.push('xmlns="'+element.namespaceURI+'"'); }
	}
...
DOMParser.prototype._parseAttribute = function(ch) {
	...
	var value = this._parseUntil(separator);
...
DOMParser.prototype._unescapeEntities = function(str) {
	var s = str
			...
			.replace(/&apos;/g, "'")
			.replace(/&amp;/g, "&")
			.replace(/&#(x?)([0-9]+);/g, function(str, x, num) {
				return String.fromCharCode(parseInt(num, (x ? 16 : 10)));
			});
	var r = s.match(/&([\S]+)?/);
	if (r && r[1]) { this._error(DOMParser._ERR_ENTITY, r[0]); }
	return s;
}
