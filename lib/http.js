...
HTTP.ServerRequest.prototype._parsePOST = function() {
    ...
    if (ct.indexOf("application/x-www-form-urlencoded") != -1) {
		...
		this.post = HTTP._parseQueryString(data);
...
HTTP.ServerResponse = function(output, header) {
	this._output = output;
	this._header = header;
...
HTTP.ClientRequest.prototype._handleResponse = function(data, follow) {
	...
	if (loc.indexOf("://") != -1) { /* absolute URI, standards compliant */
		this._url = loc;
	} else { /* relative URI */
		if (loc.charAt(0) == "/") {
...
HTTP.ClientResponse = function(data) {
	...
	this.data = data.substring(index+4);
