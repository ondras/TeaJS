 ...
 * @param {char} shortOpt Short option character
...
GetOpt.prototype._parseOptionShort = function(str) {
	...
	for (var i=0;i<list.length;i++) {
		var ch = list[i];
		var id = this._findOptionShort(ch);
		if (!id) { throw new Error("Unknown option '-"+ch+"'"); }
