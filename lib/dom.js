var inherit = function(what, from) {
	var tmp = function() {};
	tmp.prototype = from.prototype;
	what.prototype = new tmp();
...
Attr.prototype.cloneNode = function(deep) {
	var attr = this.ownerDocument.createAttribute(this.nodeName);
...
var DocumentFragment = function(ownerDocument) {
	Node.call(this, ownerDocument, "#document-fragment");
...
DocumentFragment.prototype.cloneNode = function (deep) {
	var elm = this.ownerDocument.createDocumentFragment();
...
Element.prototype.cloneNode = function(deep) {
	var elm = this.ownerDocument.createElement(this.nodeName);
	if (deep) {
		for (var i=0;i<this.attributes.length;i++) {
...
inherit(CDATASection, CharacterData);

CDATASection.prototype.cloneNode = function(deep) {
	return this.ownerDocument.createCDATASection(this.nodeValue);
}
...
inherit(ProcessingInstruction, Node);
...
ProcessingInstruction.prototype.cloneNode = function(deep) {
	return this.ownerDocument.createProcessingInstruction(this.nodeName, this.nodeValue);
}
...
XMLSerializer.prototype._serializeElement = function(element, depth) {
	...
	for (var i=0;i<element.attributes.length;i++) {
		var ret = this._serializeNode(element.attributes[i], depth);
		if (ret) { arr.push(ret); }
	}
...
DOMParser.prototype._unescapeEntities = function(str) {
	...
	var self = this;
	return str.replace(/&(#(x)?)?([0-9a-z]+)(.)/ig, function (str, hash, x, entity, end) {
